...
class CountriesBloc extends Cubit<CountriesState> {
  ...
  Future<void> fetchData() async {
    ...
    try {
      ...
      final countriesData = await Isolate.run(() {
        List res = jsonDecode(req);
        if (res.isNotEmpty) {
      ...
      emit(state.copyWith(countries: countriesData, isLoading: false));
      print('RESPONSE FROM CALL $countriesData');
